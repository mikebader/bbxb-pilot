...
dta_rdmajpts <- dta_rdselect %>% 
  select(region, name, majrdid, rdtype) %>% 
  st_drop_geometry() %>% 
...
Next, we sample two points along each of the sampled major roads. We then draw a 1/4 mile buffer around each of those points and obtain all minor roads within that buffer. Before doing so, we split lines into segments at intersections between roads and then gather those newly split segments that intersect the 1/4 mile buffer. 

We then create a spatial data frame of points with the midpoint of each of those segments that we will use for route planning purposes. From this data frame, we eliminate all points along the sampled major roads. This is because we will observe the entire length within the region of those major roads. To accomplish this, we only save points that are not within a 50-foot buffer of the sampled major roads. The data of segment midpoints is saved to the file `out/sample-segment-midpoints.shp`.
...
rdxpts <- map2(
  dta_rdxsmp$geometry,
  rep(list(c(0.1, 0.9)), nrow(dta_rdxsmp)),
  ~st_line_sample(.x, sample = unlist(.y))
) %>% 
...
## Create buffer around major roads
rdmajsmpbuf <- dta_rdmajsmp %>% 
  filter(sampled) %>% 
  st_buffer(50) 

## Create segments split at intersections
ixn <- st_collection_extract(st_intersection(dta_rdxsmp), "POINT")
splitsegs <- lwgeom::st_split(dta_rdxsmp, ixn) %>% 
  st_collection_extract("LINESTRING") %>% 
  st_cast("LINESTRING")

dta_midpts <- map2(
  splitsegs$geometry,
  rep(list(c(0.5)), nrow(splitsegs)),
  ~st_line_sample(.x, sample = unlist(.y))
) %>% 
  map(~st_set_crs(.x, st_crs(splitsegs))) %>% 
  reduce(c) %>% 
  st_sf() %>% 
  mutate(
    inrdbuf = lengths(st_intersects(., rdmajsmpbuf)) > 0
  ) %>% 
  filter(!inrdbuf) %>% 
  st_join(dta_smpreg, join = st_nearest_feature)

st_write(dta_midpts, dsn = "out/sample-segment-midpoints.shp", append = FALSE)
...
ggplot() +
  ...
  geom_sf(data = dta_midpts, mapping = aes(), color = "blue", size = 0.1) +
  geom_sf(
...
```{r strt-interactive}
mapview(dta_midpts, col.region = "yellow") +
  mapview(randpt, col.regions = "red") + 
  # mapview(dta_rdmajpts) +
  mapview(dta_rdxsmp, zcol = "highway") 
...
Combine major streets and cross street samples into a single linestring spatial dataframe. Save this object, `dta_sampled`, to the file `out/sample-segments.Rdata` and to a shapefile `out/sample-segments.shp`.

```{r strt-save-segments}
...
dta_sampled %>% 
  select(-pts) %>% 
  st_write(dsn = "out/sample-segments.shp", append = FALSE)
